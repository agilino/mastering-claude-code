...
const steps = [
  { label: 'Change', x: 480, y: 66 },
  { label: 'Browser', x: 780, y: 210 },
  { label: 'Observe', x: 480, y: 354 },
  ...
  <div class="w-full flex flex-col items-center gap-4">
    <svg viewBox="0 0 960 420" width="960" height="420" class="w-full max-w-3xl h-auto max-h-full">
      ...
      <g v-for="s in steps" :key="'n' + s.label">
        <circle :cx="s.x" :cy="s.y" r="56" fill="var(--na-bg-raised)" stroke="var(--na-primary-400)" stroke-width="2" />
        <text :x="s.x" :y="s.y + 6" font-weight="600" fill="var(--na-fg)" text-anchor="middle" style="font-size:16px">{{ s.label }}</text>
      </g>
