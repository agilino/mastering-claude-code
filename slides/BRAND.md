...
fetched directly, no headless browser needed) while building the original black-belt deck.
...
palette** for projection legibility in a training room, using that same zinc-900/950
...
`public/brand/adam-furmanczuk.webp` — Adam Furmanczuk's profile photo from
`https://www.nextacademy.io/trainers/adam`, fetched through the site's own image proxy
(`/api/blob?url=…&w=1080&q=85`; the underlying blob store is private). 400×400, the largest
size the site serves. Role line and bio facts on the trainer slide are taken verbatim from
that page. LinkedIn: `https://www.linkedin.com/in/adam-agilino/`.
