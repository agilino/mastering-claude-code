...
- Say: "notice this is not a config file. You are describing an org chart."
...
- Correct two things: teammates message each other BY NAME through the SendMessage tool — no @-mention syntax between peers
...
- Say before sending: this is prose, not a script — Claude is about to write the .js file from it
...
- Say the token cost out loud — workflows are the most expensive of the three, the price of bounded roles, clean context per agent, and a deterministic review gate
...
- Say out loud: the script does NOT land in .claude/workflows/ on its own — it's written under ~/.claude/projects/<session-dir>/ first
