...
- Einmal klar sagen: nicht mehr bauen, jetzt kontrollieren
...
- Sagen: "Wir starten hier, weil das die Zeile ist, die immer im Spiel ist."
...
- Sagen: ab jetzt nach jeder Task auf genau diesen Befehl zurückkommen
- [click] Das Diagramm ist dasselbe Bild, das der Befehl als Text zeichnet

<!-- @note: budget-or-dumping-ground -->
- Demo: beide Dateien live öffnen — das ist der gesamte Inhalt, nichts versteckt
- Klar sagen: es gibt nichts zu kürzen
...
- Gut zu wissen: seit Claude Code v2.1.277 liest Claude AGENTS.md von selbst — aber nur, wenn das Repo keine CLAUDE.md hat. CLASH hat eine, also lädt weiterhin der @AGENTS.md-Import die AGENTS.md. Den Import in der CLAUDE.md behalten, die wir schreiben
...
- Wenn ein Entwurf vor allem aus Prosa und Bauchgefühl besteht, dagegenhalten: "Könnte ein Hook das erzwingen? Wenn nicht, ist es eine Regel oder eine Präferenz?"
...
- Klar sagen: der Unterschied zwischen den beiden Spalten ist kein schlaueres Modell — es ist dasselbe Modell, gezielt eingesetzt

<!-- @note: plan-mode-review-first -->
- Demo: live in den Plan Mode wechseln (Shift+Tab, bis plan dasteht)
- Das nächste Feature beschreiben: Echtzeit-Benachrichtigungen — laden aktuell beim Rendern über `getNotifications` und `getUnreadCount` in `lib/data/notifications.ts`
- Sagen: "Don't write any code yet. Propose an approach and the files it touches."
